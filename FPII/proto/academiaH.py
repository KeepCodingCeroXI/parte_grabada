class Persona():
    ...
    def alta_asignatura(self, asignatura):
        if not isinstance(asignatura, Asignatura):
            raise Exception("{} debe ser un objeto de tipo Asignatura".format(asignatura))

        if asignatura in self.asignaturas:
            return
        self.asignaturas.append(asignatura)

    def asiste_a_clase(self):
        self._horas_mensuales += 1

    @property
    def horas_mensuales(self):
        return self._horas_mensuales
...
class Profesor(Persona):
    ...
    def sueldo(self):
        return self.sueldo_base + len(self._horas_mensuales) * 10
